...
def fetch_data():
    ...
    for row in rows:
        if len(row) < 13: # Pad row if shorter than index M
            row = row + [""] * (13 - len(row))
        
        row_date = str(row[0]).strip()
        done_date = str(row[12]).strip()
        total_items = str(row[5]).strip()
        ...
        # 5. Detailed Rows for Template 1: Include if Done date == target_date OR Done date == "Pending"
        if done_date == target_date or done_date.lower() == 'pending':
